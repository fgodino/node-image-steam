...
function Processor(options, storage) {
  ...
  if (this.options.sharp) {
    if (this.options.sharp.cache) {
      sharp.cache(this.options.sharp.cache);
    }
    if (this.options.sharp.concurrency) {
      sharp.concurrency(this.options.sharp.concurrency);
    }
    if (this.options.sharp.simd) {
...
p.process = function(originalImage, imageSteps, cb) {
  ...
  var tasks = [
    function(cb){ process.nextTick(getMetaDataTask(context, context.originalImage).bind(this, cb)) },
    function(cb){ process.nextTick(getProcessorTask(context, imageSteps).bind(this, cb)) }
  ];
...
function getProcessorTask(context, imageSteps) {
  return function(cb) {
    ...
    //clone info to store originalImageInfo separatelly
    context.processedImage.info = _.merge({}, context.processedImage.info);
    ...
    try {
      async.eachOfSeries(imageSteps, function(step, index, cb){ 
        var stepProcessor = stepProcessors[step.name];

        function deleteOriginalAfterFirst(err, results){
          if(index > 0) delete context.processedImage.info.originalImageInfo;
          cb(err, results);
        }

        if (!stepProcessor) return void cb(null); // do not process
        stepProcessor(context, step, deleteOriginalAfterFirst);
        if(stepProcessor.length === 2) deleteOriginalAfterFirst(null);
      }, function(err){
        if(err){
          return void cb(err);
        }
    ...
    function processSharp(){
      context.sharp.toBuffer(function(err, outputBuffer, info) {
        ...
        context.processedImage.info.originalImageInfo = context.originalImage.info;
