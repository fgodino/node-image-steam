...
module.exports = function(context, stepInfo) {
  ...
  //Make changes relative to original image, not to optimized
  var xFactor = 1, yFactor = 1;
  if(img.info.originalImageInfo){
    xFactor = img.info.width / img.info.originalImageInfo.width;
    yFactor = img.info.height / img.info.originalImageInfo.height;
  }
  ...
  if(xFactor !== 1 || yFactor !== 1){
    stepInfo.width = Math.floor(stepInfo.width * xFactor);
    stepInfo.height = Math.floor(stepInfo.height * yFactor);
    stepInfo.left = Math.floor(stepInfo.left * xFactor);
    stepInfo.top = Math.floor(stepInfo.top * yFactor);
  }
